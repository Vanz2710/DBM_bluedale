<template>
  <div class="page">
    <div class="page-head">
      ...
      <div class="page-head-actions">
        <button v-if="allUnread.length > 0" class="btn-mark-all" @click="markAllRead" :disabled="marking">
          <svg v-if="!marking" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="display:inline;vertical-align:middle;margin-right:4px"><polyline points="20 6 9 17 4 12"/></svg>
    ...
    <!-- Admin user filter (only for reminders, not announcements) -->
...
async function sendRead(items) {
  ...
  await api.post('/v1/reminders/read', {
    items: items.map(i => ({ type: i.source_type ?? i.type, id: i.id })),
  });
}
...
.btn-mark-all {
  ...
  box-shadow: 0 6px 18px -6px rgba(29,78,216,0.55); transition: background 0.15s;
}
...
.toolbar {
  ...
  margin-bottom: 18px; border: 1px solid var(--border-soft);
  display: flex; gap: 10px; align-items: flex-end;
}
.filter-group { display: flex; flex-direction: column; gap: 5px; }
.filter-group label { font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.7px; color: var(--text-3); }
.filter-group select {
...
.section-card {
  ...
  border: 1px solid var(--border-soft); margin-bottom: 16px; overflow: hidden;
}
...
.sec-icon  { display: inline-flex; align-items: center; }
.sec-title { font-size: 14px; font-weight: 700; color: var(--text-1); }
.sec-badge { font-size: 11.5px; font-weight: 700; padding: 3px 10px; border-radius: 999px; }
