    ...
    <template v-else-if="contact">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <div v-if="contact.incharges?.length" class="pic-edit-actions">
            <button v-if="!editingPics" type="button" class="btn btn-pic-edit" @click="startPicEdit">Edit PIC</button>
            <template v-else>
              <button type="button" class="btn btn-pic-cancel" :disabled="savingPics" @click="cancelPicEdit">Cancel</button>
              <button type="button" class="btn btn-pic-save" :disabled="savingPics" @click="savePicDetails">
                {{ savingPics ? 'Saving...' : 'Save PIC' }}
              </button>
            </template>
          </div>
        </div>
          ...
          <tbody>
            <tr v-for="pic in contact.incharges" :key="pic.id">
              <td>
                <input
                  v-if="editingPics"
                  v-model="picDrafts[pic.id].name"
                  class="pic-input pic-name-input"
                  placeholder="Person in charge name"
                >
                <strong v-else>{{ pic.name }}</strong>
              </td>
              <td>
                <input
                  v-if="editingPics"
                  v-model="picDrafts[pic.id].email"
                  type="email"
                  class="pic-input"
                  placeholder="Email"
                >
                <a v-else-if="pic.email" :href="`mailto:${pic.email}`" class="contact-email">{{ pic.email }}</a>
                <span v-else class="muted">—</span>
              </td>
              <td>
                <input
                  v-if="editingPics"
                  v-model="picDrafts[pic.id].phone_mobile"
                  class="pic-input"
                  placeholder="Mobile"
                >
                <template v-else>{{ pic.phone_mobile || '—' }}</template>
              </td>
              <td>
                <input
                  v-if="editingPics"
                  v-model="picDrafts[pic.id].phone_office"
                  class="pic-input"
                  placeholder="Office"
                >
                <template v-else>{{ pic.phone_office || '—' }}</template>
              </td>
            </tr>
          </tbody>
        </table>
...
const editingPics = ref(false);
const savingPics = ref(false);
const picError = ref('');
const picDrafts = ref({});
...
function startPicEdit() {
  picError.value = '';
  picDrafts.value = Object.fromEntries(
    (contact.value?.incharges ?? []).map((pic) => [pic.id, {
      name: pic.name ?? '',
      email: pic.email ?? '',
      phone_mobile: pic.phone_mobile ?? '',
      phone_office: pic.phone_office ?? '',
    }])
  );
  editingPics.value = true;
}

function cancelPicEdit() {
  picError.value = '';
  editingPics.value = false;
  picDrafts.value = {};
}

async function savePicDetails() {
  savingPics.value = true;
  picError.value = '';

  try {
    await Promise.all((contact.value?.incharges ?? []).map((pic) => api.put(
      `/v1/contacts/${id}/incharges/${pic.id}`,
      {
        name: picDrafts.value[pic.id]?.name ?? '',
        email: picDrafts.value[pic.id]?.email ?? '',
        phone_mobile: picDrafts.value[pic.id]?.phone_mobile ?? '',
        phone_office: picDrafts.value[pic.id]?.phone_office ?? '',
      }
    )));

    contact.value.incharges = contact.value.incharges.map((pic) => ({
      ...pic,
      name: picDrafts.value[pic.id]?.name ?? '',
      email: picDrafts.value[pic.id]?.email ?? '',
      phone_mobile: picDrafts.value[pic.id]?.phone_mobile ?? '',
    ...
    picError.value = e.response?.data?.message ?? 'Failed to save PIC details. Please try again.';
  } finally {
    savingPics.value = false;
  }
}
...
.card-header { display: flex; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid #f1f5f9; }
.card-header .card-title { margin: 0; padding-bottom: 0; border-bottom: none; }
.pic-edit-actions { display: flex; align-items: center; gap: 8px; }
.btn-pic-edit { background: #f59e0b; color: white; }
.btn-pic-save { background: #22c55e; color: white; }
.btn-pic-cancel { background: #f1f5f9; color: #64748b; }
...
.pic-input { width: 100%; min-width: 130px; height: 34px; padding: 0 10px; border: 1.5px solid #e2e8f0; border-radius: 7px; font-size: 13px; color: #1e293b; outline: none; box-sizing: border-box; }
.pic-name-input { min-width: 260px; }
.pic-input:focus { border-color: #f59e0b; box-shadow: 0 0 0 3px rgba(245,158,11,0.12); }
